...
namespace XtermSharp {
	...
	class InputHandler {
		...
		readonly TerminalCommandHandler commandHandler;
		readonly EscapeSequenceParser parser;

		public InputHandler (Terminal terminal)
		{
			this.terminal = terminal;
			commandHandler = new TerminalCommandHandler (terminal);
			...
			parser.SetCsiHandler ('P', (pars, collect) => commandHandler.DeleteChars (pars));
			...
			parser.SetCsiHandler ('c', (pars, collect) => commandHandler.SendDeviceAttributes (pars, collect));
			...
			parser.SetCsiHandler ('n', (pars, collect) => commandHandler.DeviceStatus (pars, collect));
			...
			parser.SetCsiHandler ('r', (pars, collect) => commandHandler.SetScrollRegion (pars, collect));
			...
			parser.SetCsiHandler ('t', (pars, collect) => commandHandler.SetWindowOptions (pars));
			parser.SetCsiHandler ('u', (pars, collect) => terminal.RestoreCursor ());
			parser.SetCsiHandler ('v', (pars, collect) => commandHandler.CsiCopyRectangularArea (pars, collect));
			parser.SetCsiHandler ('y', (pars, collect) => commandHandler.DECRQCRA (pars));
			parser.SetCsiHandler ('z', (pars, collect) => {
				switch (collect) {
				case "$":
					commandHandler.CommandDECERA (pars);
					break;
				case "'":
			...
			parser.SetExecuteHandler (10, commandHandler.LineFeed);
			parser.SetExecuteHandler (11, commandHandler.LineFeedBasic);   // VT Vertical Tab - ignores auto-new-line behavior in ConvertEOL
			parser.SetExecuteHandler (12, commandHandler.LineFeedBasic);
			parser.SetExecuteHandler (13, CarriageReturn);
			parser.SetExecuteHandler (8, commandHandler.Backspace);
